...
@section('title', 'Tambah Dokumen Mutu - SIMUTU POLIJE')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="mb-1 fw-bold">Tambah Dokumen Mutu</h4>
...
<div class="card border-0 shadow-sm">
    <div class="card-body">
        <form action="{{ route('dokumen-mutu.store') }}" method="POST">
            @csrf
            <div class="row">
                <div class="col-md-6 mb-3">
                    <label for="kategori_dokumen_id" class="form-label">Kategori Dokumen <span class="text-danger">*</span></label>
                    <select class="form-select @error('kategori_dokumen_id') is-invalid @enderror" name="kategori_dokumen_id" required>
                        ...
                        <option value="{{ $kd->id }}" {{ old('kategori_dokumen_id') == $kd->id ? 'selected' : '' }}>{{ $kd->nama }}</option>
                        @endforeach
                    </select>
                    @error('kategori_dokumen_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-6 mb-3">
                    <label for="standar_mutu_id" class="form-label">Standar Mutu</label>
                    <select class="form-select" name="standar_mutu_id">
                        <option value="">-- Pilih Standar Mutu --</option>
                        @foreach($standarMutus as $sm)
                        <option value="{{ $sm->id }}" {{ old('standar_mutu_id') == $sm->id ? 'selected' : '' }}>{{ $sm->nama_standar }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select" name="unit_kerja_id">
                        ...
                        <option value="{{ $uk->id }}" {{ old('unit_kerja_id') == $uk->id ? 'selected' : '' }}>{{ $uk->nama_unit }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="d-flex gap-2">
                ...
                <a href="{{ route('dokumen-mutu.index') }}" class="btn btn-secondary">Batal</a>
            </div>
        </form>
    </div>
</div>
